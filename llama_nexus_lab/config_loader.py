    ...
    return NexusConfig(
        ...
        runtime=RuntimeConfig(
            ...
            reasoner_adapter=ReasonerAdapterConfig(
                enabled=bool(runtime.get("reasoner_adapter", {}).get("enabled", False)),
                base_url=str(runtime.get("reasoner_adapter", {}).get("base_url", "")).rstrip("/"),
                model=str(runtime.get("reasoner_adapter", {}).get("model", "")),
                timeout_s=int(runtime.get("reasoner_adapter", {}).get("timeout_s", runtime["stage_timeout_s"])),
            ),
        ),
        router_rules=rules,
